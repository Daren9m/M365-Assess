@{
    ...
    PrivateData       = @{
        PSData = @{
            ...
            ReleaseNotes = 'v2.9.3 - Big patch covering v2.7.0 closeout + v2.10.0 polish to date. ADDED: Artifact provenance (#867) -- every audience-facing output carries the M365-Assess version + timestamp; new _Assessment-Provenance.json at the assessment root with SHA-256 hashes of every artifact for chain-of-custody verification. FrameworkQuilt redesign (#751/#855) -- adaptive single/multi layout with animated donut score, sortable comparison table, coverage chart, family chart, primary CTA. HideableBlock wrapper for cards/sections in edit mode (#712). Native taxonomy for HIPAA / SOC2 / Essential Eight / SCUBA -- 12 of 14 frameworks now covered (#845 partial). Findings-table column resize + sort with localStorage persist (#846). Design handoff package committed at docs/design/framework-redesign/ (#856). docs/LEVELS.md semantic reference (#844). CHANGED: FilterBar consolidated to single flowing row, density-aware compact mode (#847) -- height drops ~250px to <=120px. CIS M365 v6 sections 4 and 9 added (Microsoft Intune, Microsoft Fabric). Topbar text-size split into A- / A+ controls (#852). FIXED: Appendix Email-auth card always reported 0/N passing for SPF/DKIM (#860). Finding-detail Current value outline now reflects status (refs #674). v2.9.2: Sequence column rename + breaking SaveBaseline switch.'
        }
    }
}
